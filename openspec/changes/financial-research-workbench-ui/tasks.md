# 任务清单：财务研究工作台 UI 改造

## 工作包 A：基线与设计系统

- [x] A1. 已审核现有 `theme.ts`、`global.css`、`AppLayout`、`Sidebar`、`HeaderBar` 与五个路由页面，记录需保留的交互和硬编码色值。
- [x] A2. 已新建全红 TDD，覆盖主题 token、研究上下文、证据过程、响应式和可访问性。
- [ ] A3. 将色彩、间距、阴影、圆角和排版 token 收敛到主题与全局样式；移除主视觉中的马卡龙渐变依赖。
- [ ] A4. 调整全局导航、顶栏与页面容器；在现有 Zustand Store 中建立仅在应用运行期间有效、刷新即重置的 `researchContext`。`HeaderBar` 在桌面端只读显示“下次提问范围”和模式，首页 `ResearchContextBar` 作为唯一编辑入口；`topN`、`agentMaxSteps` 保持为首页高级选项，不接入浏览器或后端持久化。
- [ ] A5. 复用 `AppLayout` 的 `checkHealth()`，将顶栏健康状态扩展为检查中、已就绪、暂不可用三态；初始未知状态不得误报为离线。

## 工作包 B：研究工作台

- [ ] B1. 在不改变现有请求与流式逻辑的前提下，将 `ChatPage` 拆分为研究摘要条、主研究画布、证据面板和固定输入区。
- [ ] B2. 复用现有公司选择、Agent 开关、消息、KPI、来源和输入组件，建立 `ResearchContextBar`、`ResearchWelcome`、`EvidencePanel` 等界面组件；来源分数统一标注为检索匹配度且说明为回答级证据，KPI 保留原始数值/单位，普通数值保持中性展示。
- [ ] B3. 将 `ThoughtChainDrawer` 与 `MessageBubble` 的过程预览共同重构为“分析过程”，只展示经允许字段筛选、截断及敏感字段脱敏后的工具、输入摘要、观测、状态和风险提示，不渲染 `thought` 或原始过程数据。
- [ ] B4. 在 SSE 事件进入 `chatStore` 前生成安全过程视图模型：过滤单 Agent `thought`、多 Agent `thought` 步骤内容和原始过程字段；在 `chat-sessions` 加载时清理历史遗留字段，保留用户问题和最终回答。
- [ ] B5. 保留流式回答、超时、错误、会话与滚动行为，并为工作台新增空、加载、错误和强制停止状态；研究范围仅标识下次提问，不向历史消息反向归因。

## 工作包 C：辅助页面一致性

- [ ] C1. 升级图表页为分析成果页，使用现有 `ChartData` 增加类型/数量摘要、统一卡片和空状态引导。
- [ ] C2. 升级资料库页的概览、上传和索引状态层级；只使用现有资料库 API 字段。
- [ ] C3. 升级 DAG 页的高级分析流程定位、状态语义和窄屏降级。
- [ ] C4. 升级系统状态页的分组、状态层级和错误/加载状态，保持只读能力。

## 工作包 D：质量验收与收口

- [ ] D1. 为新增或显著改造组件编写前端 TDD；先确认 RED，再实现并转 GREEN，并覆盖“选择公司/模式—流式回答—证据—分析过程—跨路由—刷新”的完整研究路径，以及旧会话过程字段清理。
- [ ] D2. 在现有开发环境中于 1440、1024、768、375 宽度完成视觉截图验收，检查上下文、阅读、证据和固定输入区；移动端固定输入区同时检查安全区域与键盘遮挡；首期不为截图验收新增自动化框架或依赖。
- [ ] D3. 验证键盘焦点、对比度、`prefers-reduced-motion`、空状态、错误状态和长内容滚动。
- [ ] D4. 执行前端全量 Vitest 与生产构建；记录既有警告但不扩大修复范围。
- [ ] D5. 更新 README、CHANGELOG、交接记录和本 TDD；仅在所有适用项通过后归档变更。

## 决策门

- [ ] G1. 开发前确认品牌强调色选择：深蓝或金融青绿（二选一）。本方案默认推荐金融青绿。
- [ ] G2. 开发前确认首期仅改前端视觉与信息架构，不新增文档公司/年度/章节等后端字段。

## 版本记录

| 版本 | 日期 | 说明 |
|---|---|---|
| v1.0 | 2026-08-25 | 创建财务研究工作台 UI 改造方案与全红 TDD。 |
| v1.1 | 2026-08-25 | 方案自查：补充会话级研究上下文、检索匹配度标签、双入口过程脱敏和移动端键盘验收。 |
| v1.2 | 2026-08-25 | 二次核查：明确刷新边界、财务单位/颜色语义、回答级证据范围、过程摘要脱敏与端到端验收路径。 |
| v1.3 | 2026-08-25 | 三次核查：补齐全局上下文语义、健康状态三态、过程数据写入前安全投影及浏览器历史会话清理。 |
