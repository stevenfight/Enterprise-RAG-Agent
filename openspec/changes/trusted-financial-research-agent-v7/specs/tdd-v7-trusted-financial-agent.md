...
| D-S14 合并人工与外部验收 | 🔴 RED | 远端 CI 通过不得替代受保护分支、负责人审查、真实辅助技术读屏、真实 Provider 与历史运行时基线验收 | 2026-09-13：PR #1 `mergeable=true`、`mergeable_state=clean`、0 条 review；`main` 分支保护端点返回 404，说明当前无可读取的保护规则。Chrome 调试未附加，真实读屏仍未执行；隔离环境仅以 placeholder Provider 配置运行。 |
