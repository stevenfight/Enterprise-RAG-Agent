...
| A3.1/0.5 复核 | 🔴 RED | 干净依赖、真实基线与真实读屏前置 | 2026-09-12：CI 工作流固定 Python 3.11/Node 20、`requirements.lock` 与 `package-lock.json`；本机 Python 3.11 的 CI 等价后端集合为 32 passed，但 `pip check` 有无关环境冲突。前端 lock-only 图可解析，但共享 node_modules 含大量 extraneous，Node 为 24；全量 Vitest/构建未在受限等待内给出完成摘要，不能表述为通过。服务器隔离栈仍为候选 `5e17993`，而本地候选为 `b95a6a9`，故不能用于当前研究工作台的真实读屏。 |
