...
| D-S18 隔离 Provider 最小真实调用 | 🔴 RED | 在隔离后端使用项目 `DashScopeProvider` 仅调用一次 qwen-turbo；不得输出密钥、不得调用 qwen-plus/qwen-max，必须能区分 Provider 成功与失败 | 2026-09-14：隔离环境 Provider 仍为 placeholder，尚无真实调用证据。 |
