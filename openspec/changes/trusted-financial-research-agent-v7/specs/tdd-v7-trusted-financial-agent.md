...
| D-S13 ChartsPage 异步门禁 | 🟢 GREEN（本地） | 图表查询异步完成前，测试不得同步断言图表标题；应等待标题渲染后再验证研究筛选结果 | 2026-09-13：GitHub PR #1 的 frontend 检查失败于 `ChartsPage.test.tsx:59`，失败日志显示标题尚未渲染。最小修复将该处同步 `getByText` 改为等待型 `findByText`，不改产品实现；本地 `vitest run src/pages/__tests__/ChartsPage.test.tsx` 为 4 passed，`vite build` 通过。远端复核待更新候选分支后执行。 |
