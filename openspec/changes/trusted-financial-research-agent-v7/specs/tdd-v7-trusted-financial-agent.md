...
| D-S17 登录浮层 Escape 关闭 | 🟡 本地 GREEN，真实浏览器 RED | 未登录状态打开登录浮层后，焦点位于用户名输入框时按 Escape 必须关闭浮层，且不得提交登录请求或改变身份 | 2026-09-14：隔离真实浏览器确认旧 `a60f061` 的 Escape 后输入仍在无障碍树，先前 JSDOM CSSMotion 离场断言为假阳性。改为显式受控打开状态后 `HeaderBar.test.tsx` 13 passed、生产构建通过；待新候选部署并再次验证无障碍树后才可 GREEN。 |
