...
| A3.1/0.5 复核 | 🔴 RED（A3.1）/🟢 GREEN（0.5） | 干净依赖、真实基线与真实读屏前置 | 2026-09-13：隔离 Python 3.11 环境以 `requirements.lock` 完整安装后 `pip check` 为通过，`compileall` 与 CI 指定后端集合为 32 passed；隔离 Node 20.19.5 安装 421 包。服务器一次性 Linux Node 20.19.5 容器以原样 `npm test` 完整通过 53 文件/287 passed（359.19s），原样 `npm run build` 通过；容器自动删除、临时目录已删除，未触及正式或现有测试栈。Windows 本机 Vitest 仍会在启动后卡住，但 Linux 结果证明候选测试可通过，故 0.5 转绿。`v5.19=a6bbad99` 是 main/候选祖先，清单与结构化事实兼容回归 13 passed；标签仅跟踪 `data/stock_data/subset.csv`。正式服务器为更早 `8f8b26b`，虽有索引元数据但时间为 2026-08-12/14、早于标签且服务器不含可解析的 v5.19 标签，故不可追溯为 v5.19。后续只读寻址确认本地同级旧项目无 v5.19 提交对象，其 v1 索引备份与当前同名备份哈希一致且时间为 2026-05-28；服务器 `/opt/data` 索引为 2026-07-18，未发现版本化归档或 Docker 卷。仍无法补足认证成功 JSON、company_registry/metadata 或无 V7 数据库夹具。服务器隔离栈仍为 `5e17993`，早于本地候选，故 A3.1 保持红色。 |
