...
| D-S14 合并人工与外部验收 | 🔴 RED（部分控制已落实） | 远端 CI 通过不得替代受保护分支、负责人审查、真实辅助技术读屏、真实 Provider 与历史运行时基线验收 | 2026-09-13：已为 main 配置严格三项检查、1 个批准审查、管理员受限、禁止强制推送/删除与对话解决。PR #1 现因缺少 review 显示 `blocked`，三项 CI 均 success，说明保护规则生效。Chrome 调试未附加，真实读屏仍未执行；隔离环境仅以 placeholder Provider 配置运行。 |
