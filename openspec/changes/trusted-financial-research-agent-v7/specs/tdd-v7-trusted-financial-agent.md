...
| D-S17 登录浮层 Escape 关闭 | 🟢 GREEN | 未登录状态打开登录浮层后，焦点位于用户名输入框时按 Escape 必须关闭浮层，且不得提交登录请求或改变身份 | 2026-09-13：先 RED（HeaderBar 12 passed / 1 failed），后改为由顶部栏统一监听 Escape；关闭动作进入 Popover 离场状态并销毁隐藏内容，`HeaderBar.test.tsx` 13 passed，`npm run build` 通过。JSDOM 不实现 CSSMotion 动画结束，组件断言验证离场已开始；仍须在隔离真实浏览器复验无障碍树实际消失。 |
