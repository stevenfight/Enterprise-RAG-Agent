...
- [ ] F1. 运行全部后端测试、前端测试、生产构建、OpenSpec 检查和乱码扫描。（2026-09-15：候选后端完整集合 592 passed、1 skipped，前端完整集合/生产构建/乱码扫描已有通过证据；已完成 4 项纯流程 metadata 迁移和 6 项小范围产品 delta 迁移，全库严格 OpenSpec 检查为 13 passed、1 failed，仅 `trusted-financial-research-agent-v7` 缺少按能力域拆分的 delta 规格，故不得勾选。）
