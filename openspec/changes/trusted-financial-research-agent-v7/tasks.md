...
- [ ] F1. 运行全部后端测试、前端测试、生产构建、OpenSpec 检查和乱码扫描。（2026-09-15：候选后端完整集合 592 passed、1 skipped，前端完整集合/生产构建/乱码扫描已有通过证据；本轮 `make-f1-tests-reproducible` 严格校验通过，但全库严格 OpenSpec 检查仍被 11 个历史活跃变更缺少 delta 规格阻塞，故不得勾选。）
