...
- [ ] F2. 运行完整金融评测、故障演练、安全集与成本对比。（2026-09-15 已完成无密钥预验收：离线评测相关回归 36 passed，来源审计/评测相关回归 39 passed，五类故障演练 5 passed，安全集 19 passed，成本/预算/路由回归 12 passed；隔离候选 `--source-root data/stock_data` 仍检出 `示例公司.pdf` 缺失，候选分支仍只有 1 条 `draft` 种子样本且 coverage `ready=false`。项目根目录虽有未跟踪的 100 条候选和可核对的 12 份 PDF，但未装配到候选/staging，且发布元数据仍为 `draft/pending_review/release_ready=false`，未达到完整金融评测发布门禁，F2 不得勾选。）
