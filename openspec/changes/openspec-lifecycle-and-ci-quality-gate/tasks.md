...
- [x] B5. 本地等价命令与 YAML 解析已通过，工作包 B 已提交（`2c40314`）；远端 actionlint 与 GitHub Actions 回归验证仍待完成。
...
- [ ] B7. 由仓库管理员在 GitHub 为 main 启用 Quality Gate 必需检查；再验证失败质量任务不发布镜像。此项需要 GitHub 管理权限或已认证 CLI，不能由本地文件替代。
- [ ] B8. 所有适用验证通过后，将本 TDD 标记为 GREEN。

## 工作包 C：自动部署可验证性

- [ ] C0. 在推送当前变更前，临时移除 `workflow_run` 触发，仅保留 `workflow_dispatch`，以隔离已失败的自动部署；记录恢复条件，不能永久关闭自动部署。
...
- [ ] C5. 仅在 C1 至 C4 通过且用户确认后，恢复 `workflow_run` 自动触发；使用 `workflow_run.head_sha` 拉取两个同版本镜像，并核验自动部署成功。
