...
class ResearchTaskAdapter:
    ...
    def record_failure(self, task_id: str, failure: ResearchFailureDecision, *, actor: str) -> RouteDecision:
    ...
    def commit_step(self, task_id: str, step_id: str, attempt_token: str, owner_token: str, expected_revision: int, *, dag_step_ids: Iterable[str], dependency_versions: dict[str, str], invocation: dict[str, Any], artifact_ids: Iterable[str] = (), fact_ids: Iterable[str] = (), now: float | None = None):
        ...
        return self.execution_store.commit_step(self.run_id_for(normalized_task_id), self._step_id(step_id), attempt_token, owner_token, expected_revision, checkpoint=checkpoint, invocation=invocation, now=now)
